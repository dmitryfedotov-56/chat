...
*/
		test_Messages();
/*
		try		// sign in error - wrong user
		{
			chat->start_Session("user", "passx");
		}
		catch (int code) { error_Message(code); };

		try		// sing in error - wrong password
		{
			chat->start_Session("user1", "passx");
		}
		catch (int code) { error_Message(code); };

		try		// sign up error - existing user
		{
			chat->create_User("user1", "pass1");
		}
		catch (int code) { error_Message(code); };

		open_Session("user1", "pass1");
		open_Session("user2", "pass2");
		open_Session("user3", "pass3");
*/
		start_Chat();
		close_Chat();
		return 0;
