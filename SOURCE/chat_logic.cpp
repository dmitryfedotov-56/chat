	...
	void Chat::open_Chat(ID id) 
	{
		...
		user_list = chat_class->create_List_Access();
	...
	ID Chat::new_User(const char* name, const char* password) 
	{
		...
		user_list->include_Element(user_id);
		return user_id;
	};
	...
	ID Chat::user_ID(const char* name)	// find user by name 
	{
		...
		ID user_id = user_list-> init_Element();
		while (user_id != NOTANID)
		{
			user-> open(user_id);
			user_name = user->allocate_Name();
			...
			free((void*)user_name);
			user_id = user_list-> next_Element();
		};
		return NOTANID;
	};
	...
	Session_Access* Chat::start_Session(const char* name, const char* password)
	{
		...
		const char* user_pass = user->allocate_Password();	// check pasword
...
*/
	void Session::send_Message(const char* recipient, const char* text) 
	{
		...
		ID message_id = message->create(text, user_id, peer_id);	// create message
	...
	void Session::broadcast_Message(const char* text) 
	{
		ID peer_id;
		ID message_id;
		message_id = message->create(text, user_id, NOTANID);		// create message
		to_User_List(message_id);									// to user list
		peer_id = chat->user_list->init_Element();					
		...
		{																
			...
			peer_id = chat->user_list->next_Element();
		};
	};
